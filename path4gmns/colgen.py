...
def perform_column_generation(column_gen_num, column_update_num, rel_gap_tolerance, ui):
    """Perform column generation for path-based User Equilibrium (UE) assignment.

    **DEPRECATED**: This function has a confusing name. Use `find_ue()` for new implementations.

    This function implements path/column-based UE in Python. For other assignment modes
    or dynamic traffic assignment (DTA), use `perform_network_assignment_DTALite()`.

    Parameters
    ----------
    column_gen_num : int
        The number of iterations for generating the column pool.

    column_update_num : int
        The number of iterations for optimizing the column pool.

    rel_gap_tolerance : float
        The relative gap tolerance to determine convergence. If None or not specified,
        no convergence check is applied.

    ui : object
        The network object generated by `pg.read_network()`.

    Returns
    -------
    tuple
        A tuple containing the total gap, relative gap, and the number of iterations
        performed during column optimization.

    Notes
    -----
    To obtain assignment results, you must call `output_columns()` and `output_link_performance()`
    after executing this function. These functions output:
    - `route_assignment.csv`: Contains paths/columns.
    - `link_performance.csv`: Contains assigned volumes and other link attributes.

    Example
    -------
    >>> total_gap, rel_gap, iterations = perform_column_generation(10, 5, 0.01, network_ui)

    Warnings
    --------
    - Ensure `column_gen_num` and `column_update_num` are non-negative.
    - Deprecated in favor of `find_ue()`.
    """
    ...
    for i in range(column_update_num):
        ...
        _update_link_travel_time(links)
        total_gap, rel_gap = _update_column_gradient_cost_and_flow(
            column_pool, links, ats, i
        )
...
def find_ue(ui, column_gen_num, column_update_num, rel_gap_tolerance=None):
    ...
    modes or dynamic traffic assignment (DTA), use `perform_network_assignment_DTALite()`.
    ...
    rel_gap_tolerance : float, optional
        The relative gap tolerance to determine convergence. If None, no convergence check is applied.

    Returns
    -------
    tuple
        A tuple containing the total gap, relative gap, and the number of iterations performed.
